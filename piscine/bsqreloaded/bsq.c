...
// this is just to test if sts_mtx is working
  ...
  max = *pos;
	ij = ft_calcularij(sts_mtx, pos, t1);
	ft_fillmap(input_map, max, ij, t1);
	ft_print_solvedmap(input_map, t1);
	return (0);
}
...
int	ft_checkcell(char **input_map, int f, int c, t_table t1)
{
	...
	while (val < maxfilas && val < maxcolumnas)
	{
		i = 0;
		while (i <= val)
		{
